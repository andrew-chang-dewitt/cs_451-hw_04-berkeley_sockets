/* * * * * * * * * * * * * * * * * * * * * * * *
 * Interface -- for client-server interactions
 * -------------------------------------------
 *
 * splits computation of step parts between main
 * loop & compute nodes. system modeled as
 * client-server relationship w/ each compute
 * node acting as a server that waits for
 * compute requests to give it the necessary
 * data (the previous step) then computes &
 * returns the result (the server's assigned
...
//   buf: { size   , step state }
// }
struct extension_data child_step(struct extension_data data) {
  // 1. declare a file descriptor for logging
  int fd = -1;
#ifdef LC_ALLOW_EXCHANGE_FD
  fd = STDOUT_FILENO;
#endif
  // dprintf(fd, "[%s:child_step()]:%d BEGIN\n", compart_name(), getuid());
...
#endif
  // dprintf(fd, "[%s:child_step)]:%d state extracted for step of size (%ld)\n",
